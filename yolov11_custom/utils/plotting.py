...
class Annotator:
    ...
    def segmentation(self, mask, label='', color=(0, 255, 0), alpha=0.0):
        """Draw polygon contour with upsampled and precise mask."""
        if isinstance(mask, torch.Tensor):
           mask = mask.unsqueeze(0).unsqueeze(0)  # shape [1, 1, H, W]
           mask = F.interpolate(mask, size=(self.im.shape[0], self.im.shape[1]), mode='bilinear', align_corners=False)
           mask = (mask.squeeze().cpu().numpy() > 0.3).astype(np.uint8)  # softer threshold

        # Extract full contours (no approximation)
        ...
        for contour in contours:
            cv2.polylines(self.im, [contour], isClosed=True, color=color, thickness=self.lw)
            if label:
               x, y = contour[0][0]
               cv2.putText(self.im, label, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 1.15, color, 2)
 



    def box_label(self, box, label: str = "", color: tuple = (128, 128, 128), txt_color: tuple = (255, 255, 255)):
    # Disabled box and label drawing
            return


    def masks(self, masks, colors, im_gpu, alpha: float = 0.5, retina_masks: bool = False):
    # Disabled shaded mask drawing
    	    return
